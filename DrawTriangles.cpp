...
static Load< void > setup_buffers(LoadTagDefault, [](){
	...
	{ //vertex array mapping buffer for color_program:
		...
		glVertexAttribPointer(
			...
			GL_FLOAT, //type
			GL_FALSE, //normalized
			sizeof(DrawTriangles::Vertex), //stride
			(GLbyte *)0 + offsetof(DrawTriangles::Vertex, Position) //offset
		);
		...
		glVertexAttribPointer(
			...
			GL_TRUE, //normalized
			sizeof(DrawTriangles::Vertex), //stride
			(GLbyte *)0 + offsetof(DrawTriangles::Vertex, Color) //offset
		);
...
DrawTriangles::DrawTriangles(glm::mat4 const &world_to_clip_) : world_to_clip(world_to_clip_) {
}

void DrawTriangles::draw(glm::vec3 const &a, glm::vec3 const &b, glm::vec3 const &c, glm::u8vec4 const &color) {
